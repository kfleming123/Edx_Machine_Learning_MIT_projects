...
def hinge_loss_single(feature_vector, label, theta, theta_0):
    ...
    """
    agree = label*np.dot(feature_vector, theta) + label*theta_0
    if agree >= 1:
        return 0
    else:
        return 1 - agree
...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    for i in range(len(feature_matrix)):
        agree = labels[i]*np.dot(feature_matrix[i], theta) + labels[i]*theta_0
        if agree < 1:
            error += (1-agree)
    return error/len(feature_matrix)
#pragma: coderesponse end
...
def perceptron_single_step_update(
    ...
    if label*((np.dot(current_theta,feature_vector)) + current_theta_0) <= 0:
        new_theta = np.add(current_theta, label*feature_vector)
...
def average_perceptron(feature_matrix, labels, T):
    ...
    for t in range(T):
        for i in get_order(feature_matrix.shape[0]):
            current_theta, current_theta_0 = perceptron_single_step_update(feature_matrix[i], labels[i], current_theta, current_theta_0)
            total_theta = np.add(total_theta, current_theta)
            total_theta_0 += current_theta_0
            updates += 1
    average_theta = total_theta/updates
    average_theta_0 = total_theta_0/updates
...
def classify(feature_matrix, theta, theta_0):
    ...
    for i in feature_matrix:
        prediction = np.dot(i, theta) + theta_0
        if prediction > 0:
...
def classifier_accuracy(
    ...
    """
    try:
        theta, theta_0 = classifier(train_feature_matrix, train_labels, kwargs['T'])
    except:
        theta, theta_0 = classifier(train_feature_matrix, train_labels, kwargs['T'], kwargs['L'])
    predictions_train = classify(train_feature_matrix, theta, theta_0)
    predictions_valid = classify(val_feature_matrix, theta, theta_0)   
    return(accuracy(predictions_train, train_labels), accuracy(predictions_valid, val_labels))
...
def bag_of_words(texts):
    ...
    """
    exclude = open("stopwords2.txt").read().splitlines()
#    with open('stopwords.txt') as fp:
#        line = fp.readline()
#        exclude = list(map(int, line.split('/n')))
